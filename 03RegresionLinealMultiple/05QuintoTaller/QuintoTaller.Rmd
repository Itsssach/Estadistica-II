...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \item (c) En el modelo de regresión lineal simple de la clase $Y_{i} = \beta_{0}+ \beta_{1}X_{i1}+ \beta_{2}I_{i1}+ ...+ \beta_{ic}I_{i,c-1}+ \varepsilon_{i}; \varepsilon_{i} \overset{iid}{\sim} N(0, \sigma^{2})$ las rectas generadas cambian su pendiente.
    \item (d) En el modelo de regresión lineal con interacción $Y_{i} = \beta_{0}+ \beta_{1}X_{i1}+ \beta_{2}I_{i1}+ ...+ \beta_{ic}I_{i,c-1}+ \beta_{11}X_{i1}I_{i1}+ \beta_{12}X_{i1}I_{i2}+ ...+ \beta_{1, c-1}X_{i1}I_{i,c-1}+ \varepsilon_{i}; \varepsilon_{i} \overset{iid}{\sim} N(0, \sigma^{2})$ las rectas generadas cambian su intercepto y pendiente.
  \end{itemize}
\end{enumerate}
...
\begin{enumerate}
\item Escriba el modelo de regresión lineal múltiple, junto con sus supuestos. Use el método de selección \textit{'Forward'} para determinar el mejor modelo (paso por paso) y compárelo con el resultado dado por el método \textit{'Backward'} y \textit{'Stepwise'}.  
\item Considere el modelo de regresión lineal dado por la interacción $Y_{i} = \beta_{0}+ \beta_{1}X_{i1}+ \beta_{2}I_{i1}+ \beta_{3}I_{i2} + \beta_{11}X_{i1}I_{i1}+ \beta_{12}X_{i1}I_{i2} + \varepsilon_{i}; \varepsilon_{i} \overset{iid}{\sim} N(0, \sigma^{2})$, empleado para evaluar el cambio de precio en automóviles en función de \textit{'Bore'} (numérica) y \textit{'BodyStyle'} (categórica), esta última con tres categorías \textit{'Sedan', 'Otros' y 'Hatchback'}. \textbf{Se toma como referencia la categoría 'Hatchback'. Responda:}
  \begin{itemize}
  ...
  \item (b) Interprete la estimación del parámetro $\beta_{11}$ en términos del problema.
  ...
  \item (d) Determine si el cambio promedio \textit{'Price'} por unidad de cambio en \textit{'Bore'} es igual para los niveles de \textit{'BodyStyle'} de la clase \textit{'Sedan' y 'Otros'}. Escriba la hipótesis correspondiente y el criterio de decisión.
  \end{itemize}
\end{enumerate}
